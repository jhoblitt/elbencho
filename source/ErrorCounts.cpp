 ...
 */
void ErrorCounts::getAsPropertyTreeForJSONFile(bpt::ptree& outTree, std::string subtreeKey) const
{
	bpt::ptree subtree;
 ...
 */
void ErrorCounts::getAsPropertyTreeForService(bpt::ptree& outTree) const
{
	for(const auto& kindCountPair : counts)
	{
		bpt::ptree listItem;
		...
		outTree.add_child(XFER_STATS_ERRCOUNTLIST_ITEM, listItem);
	}
}
 ...
 */
void ErrorCounts::setFromPropertyTreeForService(bpt::ptree& tree)
{
	counts.clear();

	auto listTree = tree.get_child_optional(XFER_STATS_ERRCOUNTLIST);
	if(!listTree)
		return;
